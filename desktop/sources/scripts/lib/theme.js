...
export function Theme(client) {
  ...
  this.open = () => {
    ...
    input.onchange = (e) => {
      ...
      const fileList = /** @type {FileList} */ (inp.files);
      this.read(fileList[0], this.load);
    };
    input.click();
  };
  ...
  this.read = (file, callback) => {
    ...
    reader.onload = (event) => {
      callback(/** @type{FileReader} */ (event.target).result);
    };
    ...
    try {
      return {
        background: /** @type{HTMLElement} */ (
          svg.getElementById("background")
        ).getAttribute("fill"),
        f_high: /** @type{HTMLElement} */ (
          svg.getElementById("f_high")
        ).getAttribute("fill"),
        f_med: /** @type{HTMLElement} */ (
          svg.getElementById("f_med")
        ).getAttribute("fill"),
        f_low: /** @type{HTMLElement} */ (
          svg.getElementById("f_low")
        ).getAttribute("fill"),
        f_inv: /** @type{HTMLElement} */ (
          svg.getElementById("f_inv")
        ).getAttribute("fill"),
        b_high: /** @type{HTMLElement} */ (
          svg.getElementById("b_high")
        ).getAttribute("fill"),
        b_med: /** @type{HTMLElement} */ (
          svg.getElementById("b_med")
        ).getAttribute("fill"),
        b_low: /** @type{HTMLElement} */ (
          svg.getElementById("b_low")
        ).getAttribute("fill"),
        b_inv: /** @type{HTMLElement} */ (
          svg.getElementById("b_inv")
        ).getAttribute("fill"),
      };
    } catch (err) {
